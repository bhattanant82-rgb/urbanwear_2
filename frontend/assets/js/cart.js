(function () {
  ...
  function ensureMiniCartDOM() {
    ...
    panel.innerHTML = `
      ...
      <div class="mini-cart-footer" id="mini-cart-footer" style="display:none;">
        ...
        <button class="btn-checkout-mini" onclick="window.location.href='checkout.php'">Proceed to Checkout</button>
        <a href="cart.php" class="view-full-cart">View Full Bag</a>
      </div>`;
    ...
    if (cart.length === 0) {
      itemsEl.innerHTML = `
        <div class="empty-mini-cart">
          ...
          <a href="men.php" class="mc-shop-btn">Start Shopping</a>
        </div>`;
    ...
    cart.forEach(item => {
      ...
      html += `
        <div class="mini-cart-item" data-id="${item.id}">
          <a href="product.php?id=${item.id}" class="mc-item-img-link">
            <img src="${imgSrc}" alt="${item.name}" loading="lazy">
          </a>
          <div class="mc-item-info">
            <a href="product.php?id=${item.id}" class="mc-item-name">${item.name}</a>
  ...
  window.mcChangeQty = function (id, delta) {
    ...
    if (window.IS_LOGGED_IN && window.AUTH_TOKEN) {
      const root = window.API_ROOT || `${location.protocol}//${location.hostname}:5000`;
  ...
  window.refreshCartFromBackend = async function () {
    if (!window.IS_LOGGED_IN || !window.AUTH_TOKEN) return null;
    const root = window.API_ROOT || `${location.protocol}//${location.hostname}:5000`;
    try {
